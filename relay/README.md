...
## حالت «زنده لحظه‌ای مستقیم در مرورگر» (روش پیشنهادی)

بعد از اینکه رله بالا آمد، سایت را طوری تنظیم کنید که مرورگر کاربر **مستقیم** به رله وصل شود.
در این حالت داده هر **۶ ثانیه** یک‌بار از TSETMC خوانده می‌شود (نه ۱۵ دقیقه‌ای) و پنل هر نماد هم هر ۸ ثانیه رفرش می‌شود.
هیچ کد اضافه‌ای لازم نیست — فقط این دو متغیر:

| متغیر | کجا ست می‌شود | مقدار |
|---|---|---|
| `TSETMC_RELAY` | GitHub Secrets (و Freebuff deploy env) | آدرس رله، مثل `https://دامنه-شما.ir/relay` |
| `PUBLIC_RELAY_KEY` | GitHub Secrets | همان مقدار `RELAY_SECRET` (اگر گذاشته‌اید) |
| `RELAY_SECRET` | روی خود سرور/هاست رله | توکن دلخواه |

`TSETMC_RELAY` در فایل `api/config.json` منتشر می‌شود و فرانت‌اند خودش آن را می‌خواند.

### ساده‌ترین راه فعال‌سازی: فایل `relay-config.json`

نیازی به تنظیم Secrets نیست. کافی است فایل `relay-config.json` در ریشه ریپو را باز کنید و
آدرس رله را در آن بگذارید:

```json
{ "relay": "https://دامنه-شما.ir/relay", "relayKey": "" }
```

بعد کامیت کنید. در اولین ساخت، سایت خودکار به داده زنده لحظه‌ای سوئیچ می‌شود.
متغیرهای محیطی (Secrets) هم پشتیبانی می‌شوند و بر این فایل **اولویت** دارند.

### ⚠️ نکته مهم: رله باید HTTPS باشد

سایت روی GitHub Pages با **HTTPS** سرو می‌شود و مرورگرها اجازه نمی‌دهند یک صفحه‌ی HTTPS
به یک آدرس `http://<IP>:8787` درخواست بزند (خطای Mixed Content). پس:

- **هاست اشتراکی + دامنه**: خودش HTTPS دارد → بهترین و ارزان‌ترین گزینه ✅
- **VPS با IP خام**: باید دامنه بگیرید و با nginx + Certbot روی `https` بیاورید،
  یا سریع‌تر: `cloudflared tunnel --url http://127.0.0.1:8787` و آدرس `https://...trycloudflare.com` را
  به‌عنوان `TSETMC_RELAY` بگذارید.

### چطور مطمئن شوم کار می‌کند؟

روی خود سایت، نشان (badge) منبع داده یکی از این سه حالت را نشان می‌دهد:

- 🟢 **داده زنده لحظه‌ای (مستقیم از TSETMC)** → رله درست کار می‌کند
- ⚪ **داده واقعی (به‌روزرسانی ۱۵ دقیقه‌ای)** → رله وصل نیست؛ اسنپ‌شات گیت‌هاب استفاده می‌شود
- 🟣 **حالت نمایشی (دمو)** → هنوز هیچ داده‌ی واقعی نرسیده است

> اگر رله یک لحظه قطع شود، سایت **خطا نمی‌دهد**: خودکار به داده‌ی اسنپ‌شات برمی‌گردد و
> به‌محض برگشتن رله، دوباره لحظه‌ای می‌شود.
